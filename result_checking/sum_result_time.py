...
def time_str_to_seconds(time_str):
    ...
    minutes = int(parts[1])
    seconds = int(parts[2])
    total_seconds = hours + minutes/60 + seconds/3600
...
for paras in tuple_list:
    ...
    if os.path.isfile('allt/'+ file_name + '.iqtree'):
        invar = 0 
        with open('allt/' + file_name + '.iqtree') as b:
            for line in b.readlines():
        ...
        with open('allt/' + file_name + '.log') as b:
            for line in b.readlines():
                if 'chosen according to BIC' in line:
                    model_o = line.split()[2]
                    break
                
        if os.path.isfile('allt/'+ file_name + '_rf.rfdist'):
            with open('allt/' + file_name + '_rf.rfdist') as b:
                for line in b.readlines():
                    if 'Tree0' in line:
        ...
        #time and mem
        if os.path.isfile('allt/'+ file_name + '_time.txt'):
            with open('allt/'+ file_name + '_time.txt') as b:
                for line in b.readlines():
                    if 'Elapsed (wall clock) time (h:mm:ss or m:ss):' in line:
                        timem = time_str_to_seconds(line.split()[-1])
                    if 'Maximum resident set size (kbytes):' in line:
                        memm = float(line.split()[-1])/1024
        ...
        result_row = result_row + [optimal, str(invar), str(rate_o), str(rate_re), change, tree_length, nrfm, llhm, bicm, timem, memm, str(ise_q), str(ise_f)]
    else:
        #print(paras, 'method' + str(i))
        result_row = result_row + ['','','','','','','','','','','','']
        
    if os.path.isfile('gtrt/'+ file_name + '.iqtree'):
        invar = 0 
        with open('gtrt/' + file_name + '.iqtree') as b:
            for line in b.readlines():
                if 'Best-fit model according to BIC:' in line:
                    optimal = line.count(',') + 1
                    model_re = line.split()[-1]
        ...
        with open('gtrt/' + file_name + '.log') as b:
            for line in b.readlines():
                if 'chosen according to BIC' in line:
                    model_o = line.split()[2]
                    break
        
        if os.path.isfile('gtrt/'+ file_name + '_rf.rfdist'):
            with open('gtrt/' + file_name + '_rf.rfdist') as b:
                for line in b.readlines():
                    if 'Tree0' in line:
        ...
        #time and mem
        if os.path.isfile('gtrt/'+ file_name + '_time.txt'):
            with open('gtrt/'+ file_name + '_time.txt') as b:
                for line in b.readlines():
                    if 'Elapsed (wall clock) time (h:mm:ss or m:ss):' in line:
                        timeg = time_str_to_seconds(line.split()[-1])
                    if 'Maximum resident set size (kbytes):' in line:
                        memg = float(line.split()[-1])/1024
        ...
        result_row = result_row + [optimal, str(invar), str(rate_o), str(rate_re), change, tree_length, nrfg, llhg, bicg, timeg, memg,str(ise_q),str(ise_f)]
    else:
        #print(paras, 'method' + str(i))
        result_row = result_row + ['','','','','','','','','','','',''] 
