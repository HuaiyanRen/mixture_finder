...
# run class 1
    ...
    for i in range(len(type_dict)): # run all model types in class 1
        ...
        print('class:'+ str(classes), type_dict[i], score_list) # list all the scores generated

        score_list_loop.append(np.min(score_list)) # get the lowest score of 10 repeatitions
...
# once the score increasing, return the nubmer classes with lowest score.
    return 'the optimal number of classes is:'+ str(classes-1)
